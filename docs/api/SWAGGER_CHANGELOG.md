...
## 2026-08-13 - 모집글 거래 과정 단순화

### 변경 요약

- 게시글 상태 변경 요청과 목록 필터 enum에서 `in_progress`를 제거했다.
- 게시글 상태는 `open -> closed -> completed` 순서로 변경한다.
- 참여자 응답에 단계 번호, 전체 단계 수, 다음 상태, 다음 행동 문구와 행동 주체를 추가했다.
- 모집글 상세 응답에 `participantProcessGuide`를 추가했다.
- 참여자 상태 표시명을 참여 신청, 참여 확정, 입금 확인, 수령완료로 정리했다.

### 호환성

기존 `in_progress` 데이터는 완료 또는 취소할 수 있다. 신규 상태 변경 요청에서는
`in_progress`를 허용하지 않는다.
...
## 2026-08-13 - 비밀번호 입력 길이 제한 추가

- 브랜치: `feat/password-20-char-limit`
- 회원가입·로그인·사용자 수정 요청의 비밀번호 길이를 8자 이상 20자 이하로 제한했다.
